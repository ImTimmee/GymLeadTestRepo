...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!supabaseUrl || !serviceKey) {
    return res.status(500).json({ error: "Missing env vars: VITE_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY" });
  }
  ...
  return res.status(200).json({ flow, questions: questions ?? [], profile: profile ?? null });
}
